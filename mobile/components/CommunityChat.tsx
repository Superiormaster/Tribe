...
export default function CommunityChat({ communityId }: Props) {
  /*
   * Imports are intentionally unchanged.
   * React Native primitives are obtained here so the existing import
   * section remains exactly as supplied.
   */
  const {
    View,
    Alert,
  ...
  /*
   * Native files are represented by the already-converted upload system.
   * Keep this state compatible with the existing child component contract.
   */
  ...
  const messageBodyRef =
    useRef<CommunityMessageBubblesHandle | null>(null);
  ...
  const [showMuteModal, setShowMuteModal] =
    useState(false);
  ...
  const [replyingTo, setReplyingTo] =
    useState<ReplyMessage | null>(null);

  const [loading, setLoading] =
    useState(false);

  const [jumpLoading, setJumpLoading] =
    useState(false);

  const jumpingToMessageRef =
    useRef(false);
  ...
  const [chatLocked, setChatLocked] =
    useState(false);
  ...
  /*
   * chatEndRef existed only for the browser DOM.
   *
   * The actual message scrolling/jumping is handled by
   * CommunityChatBody through messageBodyRef.
   */
  ...
  useEffect(() => {
    ...
    const loadCommunity = async () => {
      ...
      try {
        const community = await apiRequest(
   ...
   *
   * AppState is the React Native equivalent.
   */
  useEffect(() => {
    const socket = socketRef.current;
  ...
  /*
   * Give the community socket its state handlers.
   */
  useEffect(() => {
    setHandlers({
      setMessages,
  ...
  /*
   * Browser window.scrollTo() is intentionally removed.
   *
   * CommunityChatBody owns the native virtualized message list.
   * When replying starts, its own message positioning can be used
   * without attempting to manipulate the browser document.
   */
  useEffect(() => {
    if (!replyingTo) {
      return;
    ...
    /*
     * The message list is controlled by CommunityChatBody.
     * Keep the effect so replying remains a state transition
     * without using browser-only scrolling APIs.
     */
  }, [replyingTo]);
  ...
  const handlePinCommunityMessage =
    async () => {
      const message =
  ...
  const handleUnpinCommunityMessage =
    async () => {
      const message =
        getCurrentSelectedCommunityMessage();
  ...
  const selectedMessage =
    selectedCommunityMessages.length ===
    1
  ...
  return (
    <View className="flex-1 bg-gray-300 dark:bg-gray-900">
      ...
      <View className="flex-1 min-h-0">
        ...
        {/*
         * Kept as a native View instead of a DOM div.
         * CommunityChatBody remains responsible for scrolling.
         */}
        <View ref={chatEndRef} />
      </View>
      ...
      <CommunityOptionsModal
        ...
        onSearch={() => {
          ...
          // Message search remains
          // owned by the converted
          // CommunityOptionsModal flow.
        }}
      ...
      <ChatSelectionBar
        ...
        onPin={
          handlePinCommunityMessage
        }

        onUnpin={
          handleUnpinCommunityMessage
        }
