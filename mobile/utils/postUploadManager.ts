...
function isFile(
  ...
  const file =
    value as Partial<NativeUploadFile>;
...
class PostUploadManager {
  ...
  private async processJob(
    ...
    console.log(
      "[PostUploadManager][JOB][2] Loading latest job from SQLite",
      {
        jobId:
          originalJob.job_id,
      },
    ...
    if (!job) {
      console.error(
        "[PostUploadManager][JOB][STOP] Job NOT FOUND in SQLite",
        {
          jobId:
            originalJob.job_id,
        },
  ...
  private async prepareMediaFile(
    ...
    if (
       ...
       *
       * Network failure during upload therefore
