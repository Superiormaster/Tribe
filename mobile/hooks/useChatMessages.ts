...
/*
 * React Native-safe client ID.
 *
 * We only need a unique client identifier here.
 * This avoids relying on browser crypto APIs.
 */
...
export function useChatMessages({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (replyMessage) {
          console.log(
            "🧪 [DIRECT TEST] Server message 49:",
            {
              id:
                replyMessage.id,
              client_id:
                replyMessage.client_id,
              reply_to:
                replyMessage.reply_to,
            }
          );
        }
        ...
        if (
          ...
          console.log(
            "🟢 [INIT] AFTER DATABASE SAVE:",
            freshCachedMessages.map(
              (m: any) => ({
                id: m.id,
                client_id:
                  m.client_id,
  ...
  const sendMessage =
    async (
      ...
      console.log(
        "🚀 [SEND 2] useChatMessages.sendMessage"
      );
