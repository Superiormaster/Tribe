...
export function useTribes(
  ...
  const [showAllTribes, setShowAllTribes] =
    useState(false);

  const [loadingTribes, setLoadingTribes] =
    useState(false);

  const [tribesError, setTribesError] =
    useState<any>(null);
  ...
  const currentTribe = useMemo(
    () =>
      tribes.find(
        (tribe: any) =>
          tribe.id === selectedTribe
      ) ?? null,
    [tribes, selectedTribe]
  );
